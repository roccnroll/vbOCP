...
indipendentemente per stato e aggiunto. Questa base alimenta direttamente
la PODNN vera (train_reduced_nn.py) - l'aggregated space (eq. 18 del
paper, vedi pod.build_aggregated_pod_basis) NON e' usato qui: forzerebbe
l'aggiunto a un N molto piu' grande del suo naturale (dominato dallo
stato, che tipicamente ne richiede assai di piu'), peggiorando di molto
la regressione della PODNN sull'aggiunto senza alcun beneficio, dato che
questa pipeline e' non intrusiva (nessuna proiezione di Galerkin
accoppiata da stabilizzare - vedi handout). L'aggregated space resta
usato solo per la POD "pura" senza rete (sweep_pod_error.py).
